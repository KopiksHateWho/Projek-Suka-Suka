...
## 2026-03-10 - Cross-Page Deep Linking & Navigation UX
**Learning:** In a vanilla JS multi-page application, using URL hashes coupled with a delayed initialization (e.g., 300ms) is an effective way to trigger modals or scroll-to-actions when navigating between pages. This preserves the "SPA feel" for features like "History" or "Request Game" that only exist as modals on the home page.
**Action:** Use a `handleDeepLink` function on `window.onload` and `hashchange`. Proxy sub-page navigation clicks through functions that either open the local modal or redirect to the home page with the appropriate hash (e.g., `index.html#history`).
