...
## 2026-02-13 - Data Portability & Receipt Accessibility
**Learning:** Users often need to take order IDs or transaction hashes from mobile-first web apps to other platforms (like WhatsApp or email). Providing a one-click "Copy" action next to critical data points, paired with immediate visual feedback (toasts), drastically reduces friction.
**Action:** Identify unique identifiers or transaction data (Order IDs, Promo Codes) and add a secondary copy button. Ensure these buttons have descriptive ARIA labels to distinguish them from other actions.
