...
## 2026-02-23 - Password Visibility & Form Sanitization
**Learning:** Password visibility toggles significantly reduce friction during login/registration by allowing users to verify their input. In a vanilla JS/Tailwind project, placing the toggle inside a relative container wrapping only the input prevents layout disruption. Additionally, "ghost" conflict markers left in templates (from failed merges) can break entire sub-page layouts and navigation logic.
**Action:** Implement password toggles in a dedicated relative wrapper for the input. Regularly audit templates for residual Git conflict markers (<<<<<<<, =======, >>>>>>>) which can cause malformed HTML and broken DOM trees.
