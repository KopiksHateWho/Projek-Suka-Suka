...
## 2026-03-01 - Tailwind vs. Custom CSS for Absolute Positioning
**Learning:** Even in projects with mixed CSS, using Tailwind's `top-1/2 -translate-y-1/2` utilities is more robust for centering/positioning interactive elements (like password toggles) than "magic numbers" in custom CSS. It avoids alignment regressions across different form field heights and browsers.
**Action:** Prefer Tailwind's positioning pattern for vertical centering inside relative containers. Always wrap inputs and their interactive siblings (icons, toggles) in a `relative` wrapper to localise the coordinate system and ensure consistent layout.
