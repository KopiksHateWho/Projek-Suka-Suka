...
## 2026-05-18 - Password Visibility & Modal Entrapment
**Learning:** Providing a password visibility toggle is a fundamental UX expectation that also aids accessibility by allowing users to verify their input. Additionally, hidden or "backdoor" modals must still adhere to accessibility standards, including having an explicit 'Close' button to prevent user entrapment.
**Action:** Always include a visibility toggle for password fields. Ensure every modal has a clear, accessible close button, regardless of how the modal is triggered.
