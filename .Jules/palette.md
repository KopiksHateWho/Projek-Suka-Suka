...
## 2026-03-01 - Global Utilities & Deep Linking
**Learning:** In multi-page vanilla applications, placing shared UX utilities (like password toggles) and navigation proxies in a global script (like `auth.js`) ensures they are available before page-specific logic loads. Deep linking via URL hashes combined with a small initialization delay (e.g., 300ms) provides a seamless way to trigger modals when returning from sub-pages.
**Action:** Centralize shared UI utilities in a global script. Use hash-based routing with initialization buffers to handle cross-page feature triggers.
