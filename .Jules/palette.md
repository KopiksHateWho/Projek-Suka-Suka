...
## 2026-02-13 - Visual Feedback & Global Accessibility
**Learning:** Micro-interactions like a simple checkmark indicator (✓) provide immediate, delightful confirmation of user choice in selection-heavy UIs. Furthermore, centralized accessibility listeners (for role="button" keyboard support) should reside in global scripts to ensure consistent behavior across multi-page applications.
**Action:** Always provide visual confirmation for selections. Centralize keyboard event listeners in global scripts (e.g., auth.js) so that all pages benefit from accessibility improvements consistently.
