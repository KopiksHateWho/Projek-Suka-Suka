...
## 2026-02-23 - Keyboard Shortcut Discoverability
**Learning:** For non-obvious keyboard shortcuts (like '/' for search), adding a small visual hint (e.g. `[/]`) that is visible on hover/idle but non-obtrusive (e.g. hidden when focused or typing) significantly improves UX discoverability without cluttering the UI.
**Action:** When implementing global keyboard shortcuts, pair them with `aria-keyshortcuts` and a visual hint. Use CSS 'peer' or JS listeners to manage hint visibility during active interactions (focus/typing) to maintain a clean interface.
