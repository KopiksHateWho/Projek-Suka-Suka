...
## 2026-05-15 - Global Password Visibility & Accessibility
**Learning:** For accessible labels (e.g., `aria-label`), prefer English (e.g., 'Close Modal') over regional languages to maintain standard screen reader behavior across global design systems, even if the UI text content is localized. This ensures consistency and better default behavior for assistive technologies.
**Action:** Use English for all `aria-label` and `aria-description` attributes. Ensure every modal has a clear, accessible close button.
