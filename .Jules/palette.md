...
## 2026-03-05 - CSS-Only UI Hints
**Learning:** Using Tailwind's `peer` utilities combined with `placeholder-transparent` allows for robust, CSS-only UI state management for hints and shortcuts. This reduces JavaScript overhead and prevents layout shifts compared to manual DOM manipulation or event listeners for simple visual toggles.
**Action:** For transient UI hints (like keyboard shortcut indicators), prefer `peer-focus:invisible` or `peer-[:not(:placeholder-shown)]:invisible` on sibling elements rather than toggling classes via JavaScript.
