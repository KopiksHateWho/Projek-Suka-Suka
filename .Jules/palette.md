...
## 2026-06-25 - Navigation Proxy Pattern & Selection A11y
**Learning:** In multi-page vanilla JS applications, shared navigation items that trigger page-specific modals (e.g., History/Request) must use global proxy functions. These proxies should check for modal presence and redirect to the main page with a URL hash (deep-linking) as a fallback, ensuring feature availability from any page. Additionally, custom radio components using `div`s require manual `aria-checked` management and `role="radiogroup"` to be accessible.
**Action:** Use a "check-and-redirect" pattern for global navigation proxies. Ensure keyboard listeners in `app.js` handle both `button` and `radio` roles for custom interactive elements.
