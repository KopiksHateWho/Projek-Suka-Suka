...
## 2026-03-05 - Navigation Parity & Password UX
**Learning:** In multi-page static sites, navigation regressions (like missing backdrops or stale merge conflict markers) are common. Standardizing navigation across all entry points and adding accessibility-first password toggles (with ARIA and keyboard support) provides a unified, polished user experience.
**Action:** When updating global components like navbars, audit every subpage for parity. Implement password toggles using relative containers and absolute positioning to avoid layout shifts, and ensure `initAccessibility` is globally scoped to support non-semantic buttons across the site.
